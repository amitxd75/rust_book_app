...
/// A bottom sheet widget that facilitates an interactive chat session with Gemini,
/// automatically pulling context from the active webpage when available.
class GeminiChatSheet extends StatefulWidget {
  /// The bookmarks service to load and persist chat histories and preferences.
  final BookmarksService bookmarksService;

  /// Optional controller of the WebView to scrape active page content for contextual answers.
  final WebViewController? webViewController;

  /// Cached Gemini API key to initialize the UI immediately.
  final String? cachedApiKey;

  /// Cached Gemini model name.
  final String? cachedSelectedModel;

  /// Cached chat history messages.
  final List<Map<String, dynamic>>? cachedChatHistory;

  /// Cached system skill prompt if set.
  final String? cachedSkillPrompt;

  /// Creates a [GeminiChatSheet] widget.
  const GeminiChatSheet({
    super.key,
    required this.bookmarksService,
    this.webViewController,
    this.cachedApiKey,
    this.cachedSelectedModel,
    this.cachedChatHistory,
    this.cachedSkillPrompt,
  });
...
class _GeminiChatSheetState extends State<GeminiChatSheet> {
  ...
  Future<void> _loadApiKeyAndContext() async {
    ...
    _selectedModel = widget.cachedSelectedModel ??
        await widget.bookmarksService.getGeminiModel();

    // 2. Try reading API key from process environment variable
    String? key = widget.cachedApiKey ?? Platform.environment['GEMINI_API_KEY'];
    ...
    if (key != null && key.isNotEmpty) {
      ...
      // Fetch current chapter context and user's text selection in the background
      String? contextText;
      ...
      if (widget.webViewController != null) {
        ...
        try {
          final selectionResult = await widget.webViewController!
              .runJavaScriptReturningResult("window.getSelection().toString()");
          final decodedSelection = _decodeWebViewResult(selectionResult).trim();
          if (decodedSelection.isNotEmpty) {
            selectionText = decodedSelection;
            // Clear the selection highlight on the webpage immediately after capture
      ...
      await _initGemini(key, contextText);

      // If the user selected text in the reader, automatically trigger an explanation query
      if (selectionText != null && selectionText.isNotEmpty) {
        WidgetsBinding.instance.addPostFrameCallback((_) {
          if (mounted) {
            _sendPrompt('Explain this text: "$selectionText"');
          }
        });
      }
    } else {
  ...
  Future<void> _initGemini(String apiKey, String? contextText) async {
    ...
    String? skillPrompt = widget.cachedSkillPrompt;
    if (skillPrompt == null) {
      try {
        skillPrompt =
            await rootBundle.loadString('assets/rust-skills/SKILL.md');
      } catch (_) {
            ...
            _messages.addAll(history.map((m) {
              return {
                ...
                'widget': _buildFormattedMessage(
                    m['text'] as String, m['isUser'] as bool),
              };
            }));
          } else {
            // Cache welcoming message if history is empty
            final welcomeText =
                "Hi! I'm Gemini (using $_selectedModel). Ask me anything about ownership, lifetimes, or compiler errors as you read!";
            _messages.add({
              'text': welcomeText,
              'isUser': false,
  ...
  Future<void> _sendPrompt(String text) async {
    ...
    try {
      ...
      final errorText =
          "Sorry, I ran into an error generating that response: $e";
  ...
  Future<void> _clearChatHistory() async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text(
            'Are you sure you want to clear your conversation with the Gemini tutor?',
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            style:
                ElevatedButton.styleFrom(backgroundColor: AppColors.rustOrange),
    ...
    return Padding(
      ...
      child: GlassContainer(
        ...
        fillColor: const Color(
            0xFF14161C), // Solid 100% opaque obsidian dark panel to guarantee maximum scroll performance
  ...
  Widget _buildConfigView() {
    return Column(
      ...
      children: [
        ...
        // 1. Glass model selector dropdown
            ...
            Flexible(
              child: TextButton.icon(
                ...
                icon: const Icon(Icons.open_in_new,
                    size: 14, color: Color(0xFF2083A4)),
                label: const Text(
                  'Get Free Key',
                  style: TextStyle(
                ...
                child: Align(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      color: isUser
                          ? const Color(0xFF9E8CF4).withValues(alpha: 0.08)
                          : isErr
                              ? Colors.red.withValues(alpha: 0.08)
                              : const Color(0xFFDE7B3F).withValues(alpha: 0.08),
                      border: Border.all(
                        color: isUser
                            ? const Color(0xFF9E8CF4).withValues(alpha: 0.15)
                            : isErr
              ...
              children: [
                SizedBox(
                  ...
                  child: CircularProgressIndicator(
                      strokeWidth: 1.5, color: Color(0xFFFFB84D)),
                ),
                SizedBox(width: 8),
